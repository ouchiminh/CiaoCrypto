...
OUCHI_TEST_CASE(benchmark_camellia128)
{
    ...
    ciao::camellia<16> encoder{ key };
    {
        ...
        std::cout << "camellia-128 enc " <<  c / dur.count() / 1000*r << " k" << std::endl;
    }
}
#if 1
OUCHI_TEST_CASE(benchmark_camellia128_cbc)
{
    using namespace std::chrono;
    constexpr int r = 1024 * 16;
    std::vector<std::uint8_t> data(16 * 1024, 0xc5);
    std::vector<std::uint8_t> dest(16 * 1025);
    ciao::cbc<ciao::camellia<16>> encoder(key, key);
    data[0] = (unsigned char)steady_clock::now().time_since_epoch().count();
    {
        auto beg = std::chrono::steady_clock::now();
        for (auto k = 0ull; k < r; ++k) {
        ...
        duration<double, std::ratio<1, 1>> dur = std::chrono::steady_clock::now() - beg;
        std::cout << "camellia-128 cbc enc " <<  data.size() / dur.count() *r/ 1000 / 1000 << " M\n";
    }
