...
OUCHI_TEST_CASE(test_ctr_encode_decode)
{
    ...
    const char key[] =
        "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f";
    unsigned char dest[16*128] = {0,1,2,3,4,5,6,7};
    std::uint8_t src[16*128]{};
    std::memcpy(src, block, 16);
    ...
    OUCHI_REQUIRE_EQUAL(encoder.cipher(src, 16 * 128, dest, 16*128).unwrap(), 16 * 128);
    OUCHI_REQUIRE_EQUAL(decoder.inv_cipher(dest, 16*128, dest, 16*128).unwrap(), 16 * 128);
    OUCHI_CHECK_EQUAL(dest[0], 0x00);
    OUCHI_CHECK_EQUAL(dest[1], 0x11);
    OUCHI_CHECK_EQUAL(dest[15], 0xff);
    OUCHI_CHECK_EQUAL(dest[16*128-1], 0);
}
