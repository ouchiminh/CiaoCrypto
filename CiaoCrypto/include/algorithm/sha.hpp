...
public:
    ...
    {
        for (auto i = 0u; i < 8; ++i) {
            h_[i] = detail::initial_hash_value<elm_type>[i];
        }
    }
    ...
    bytes<Len/8> finalize() noexcept
    {
        bytes<Len/8> res;
        finalize(res.byte_data);
        return res;
    }
    void finalize(void* dest) noexcept
    {
        assert(dest != nullptr);
        constexpr size_t lidx = block_length - 1;
        std::uint8_t* byte_data = (std::uint8_t*)dest;
        ...
        if constexpr (sizeof(elm_type) == sizeof(std::uint64_t)) {
            unpack(0, buffer_ + last_block_m_length);
            unpack(length_ * 8, buffer_ + last_block_m_length + sizeof(elm_type));
        } else {
            unpack(length_ * 8, buffer_ + last_block_m_length);
        }
        process_block(buffer_);
        for (auto i = 0u; i < sizeof(h_) / sizeof(*h_); ++i) {
            unpack(h_[i], byte_data + (i * sizeof(elm_type)));
        }
        *this = sha{};
    }
...
private:
    void process_block(void* block) noexcept
    {
        ...
        for (auto i = 0u; i < 16; ++i) {
            w[i] = pack<elm_type>(ptr + (i * sizeof(elm_type)));
        }
